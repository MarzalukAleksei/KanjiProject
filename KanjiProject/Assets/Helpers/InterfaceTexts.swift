...
final class InterfaceTexts {
    ...
    static func infoAlertOnMainViewJLPT(_ numberOf: Int) -> String {
        "На экзамене JLPT используются кандзи входящие в список 常用漢字 (кандзи для повседневного использования). Всего их \(numberOf). Разделение на уровни относительно, поэтому нередко в тесте можно встретить кандзи уровнем выше чем текущий."
    }
